...
if __name__ == '__main__':
    ...
    for n_batch, batch in enumerate(tqdm(dl, total=steps_per_epoch)):
        ...
        genUmag = batch['genU'][:, 0]
        genUphi = batch['genU'][:, 1]
        genZpt =  batch['genZ'][:, 0]
        genZphi =  batch['genZ'][:, 1]
        recZpt =  batch['recZ'][:, 0]
        recZphi =  batch['recZ'][:, 1]
        ...
        #recoilres.compute(pt=batch['x'][:, :, 0],
        #                  phi=batch['x'][:, :, 2],
        #                  w=score*(1-batch['isolep_mask']),
        #                  y=batch['y']*(1-batch['isolep_mask']),
        #                  baseline=batch['puppi']*(1-batch['isolep_mask']),
        #                  gm=genZpt,
        #                  gmphi=genZphi)
    ...
    resolutionU = recoilres.plot(plot_path + '_U')
