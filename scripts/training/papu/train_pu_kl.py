...
if __name__ == '__main__':
    ...
    metrics = PapuMetricsKL(config.beta)
    ...
    for e in range(min_epoch,config.n_epochs):
        ...
        for n_batch, batch in enumerate(tqdm(dl, total=steps_per_epoch)):
            ...
            loss, _ = metrics.compute(yhat, y, batch['x'][:, :, 0], batch['x'][:, :, 2], batch['neutral_mask'], batch['y'], genmet, genmetphi, w=weight, m=loss_mask, plot_m=qm)
